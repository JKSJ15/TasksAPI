...
public class TaskServiceTest {
	...
	void setup() {
		...
		UsernamePasswordAuthenticationToken auth = new UsernamePasswordAuthenticationToken("jakson", null, null);
	...
	void save_returnsTaskDtoObject_wheSucessfull() {
		Mockito.when(ur.findByLogin("jakson")).thenReturn(Optional.of(user));
	...
	void update_returnsTaskDtoObject_whenSuccessful() {
		...
		Mockito.when(tr.findByIdAndUserLogin(2L, "jakson")).thenReturn(Optional.of(task));
		Mockito.when(tr.save(ArgumentMatchers.any(Task.class))).thenReturn(task);
		TaskDto updated = ts.update(2L, dto);

		Assertions.assertThat(updated).isNotNull();
		Assertions.assertThat(updated.getTitle()).isEqualTo("Macarena");
	}

	@Test
	@DisplayName("findById_throwTaskNotFoundException_whenNotFound")
	void findById_throwTaskNotFoundException_whenNotFound() {
		Mockito.when(tr.findByIdAndUserLogin(ArgumentMatchers.anyLong(), ArgumentMatchers.anyString()))
	...
	void delete_removeTask_whenSucessful() {
		BDDMockito.when(tr.findByIdAndUserLogin(ArgumentMatchers.anyLong(), ArgumentMatchers.anyString()))
				.thenReturn(Optional.of(task));
