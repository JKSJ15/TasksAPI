...
public class TaskControllerTest {
	...
	void list_returnListOfTasks_whenSucessful() {
		...
		BDDMockito.when(ts.list(ArgumentMatchers.anyString(), ArgumentMatchers.any(Status.class),
				ArgumentMatchers.any(Priority.class), ArgumentMatchers.any(Pageable.class))).thenReturn(list);

		TaskDto task = dto;
		Pageable pageable = Pageable.unpaged();
		ResponseEntity<Page<TaskDto>> find = this.tc.list(pageable, task.getTitle(), task.getStatus(),
				task.getPriority());
