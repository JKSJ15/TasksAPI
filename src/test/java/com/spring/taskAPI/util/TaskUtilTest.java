...
public class TaskUtilTest {
	public static Task returnTask() {
		return Task.builder().withCreatedAt(LocalDate.now()).withDateConcl(null)
				.withDeadline(LocalDate.now().plusDays(2)).withDescription("Walk with the Dog")
				.withPriority(Priority.MEDIUM).withStatus(Status.PENDING).withTitle("Walk With Dog").build();
	}
